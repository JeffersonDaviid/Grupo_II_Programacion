...
public class UsuarioDAC extends DataHelper {
     ...
     */
    public ResultSet getUser(String user, String contrasena, int rol) throws AppException {
        try {
            String sql = "SELECT "
                    ...
                    + " WHERE "
                    + APP.BASE_DATOS_MYSQL.USUARIO + " = '" + user + "' AND "
                    + APP.BASE_DATOS_MYSQL.CONTRASENA + " = '" + contrasena + "' AND "
                    + APP.BASE_DATOS_MYSQL.FK_ID_ROL + " = " + rol;
            return getResultSet(sql);
        } catch (Exception e) {
            throw new AppException(e, getClass(),
                    "Error en getUser(String user, String contrasena, int rol) " + e.getMessage());
        }
    }
